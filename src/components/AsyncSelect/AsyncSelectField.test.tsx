...
describe('AsyncSelectField::', () => {
  ...
  const getOptionsGetter = ( timeout = 10): jest.Mock => {
    return jest.fn().mockReturnValue(new Promise((resolve) =>
      setTimeout(() => resolve(generateOptions()), timeout)));
  };
  ...
  it('should show the loader', async () => {
    ...
    await waitFor(() =>{expect(screen.getAllByLabelText( 'Select option')).toHaveLength(4);});
  });
  ...
  it('should react on multiple validators', async () => {
    ...
    render(
      <FormWrapper>
        <AsyncSelectField defaultOptions name="test" label="test-label" validators={[email, minLength(13)]} loadOptions={getOptions} isOpen/>
      </FormWrapper>,
    );
  ...
  it('should show an error below the input', async () => {
    ...
    render(
      <FormWrapper>
        <AsyncSelectField defaultOptions name="test" label="test-label" validators={[minLength(13)]} loadOptions={getOptions} isOpen/>
      </FormWrapper>,
    );
